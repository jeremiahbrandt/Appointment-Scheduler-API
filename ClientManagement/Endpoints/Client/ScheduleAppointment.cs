...
namespace AppointmentManagerApi.Endpoints.Client
{
    public static class ScheduleAppointment
    {
        ...
        public static async Task<IActionResult> Run([HttpTrigger(AuthorizationLevel.Anonymous, "post", Route = "client/appointments")] HttpRequest req, ILogger log)
        {
            var clientService = new ClientService();
            var uid = Util.GetUid(req).Result;

            await clientService.ScheduleAppointment(uid, req);
